...
export async function GET() {
  try {
    //import current user details so Stripe can on board the user 
    const user = await currentUser()
    if (!user) return new NextResponse('User not authenticated')
//this is a fake account to test with in Stripe because using real info is daunting with stripe
    //this test is only valid for dev mode
