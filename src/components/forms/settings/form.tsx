...
//import comp dynamically only when it's needed then send it to the front end
const WelcomeMessage = dynamic(
  () => import('../settings/greetings-message').then((props) => props.default),
  {
    ssr: false,
  }
)
...
const SettingsForm = ({ id, name, chatBot, plan }: Props) => {
  const {
    ...
    deleting,
    loading,
  } = useSettings(id) //the custom hook we created to update the settings nd stuff
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        {/* this is the code snippet that users can copy in order to embed the chatbot into their websites */}
        <CodeSnippet id={id} />
